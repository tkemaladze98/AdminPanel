...
export class DashboardComponent implements OnInit {
  ...
  hotelResponse: any;
  constructor(private router: Router, private firebase: CrudServicesService) {}
  ...
  ngOnInit(): void {
    this.firebase.readAllHotel().subscribe((response: any) => {
      ...
      response.forEach((element: any) => {
        let roomIndex = 0;
        let hotelData = element.payload.doc.data();
        hotelData.rooms.forEach((room: any) => {
          roomIndex += 1;
          room.reserveDates.forEach((dates: any) => {
            let start = new Date(dates.start.seconds * 1000).toDateString();
            let end = new Date(dates.end.seconds * 1000).toDateString();
            this.reservedInfo = {
